...
class AddressAdmin(NormalizedSearchAdminMixin, admin.ModelAdmin):
    list_display = ["title", "user", "full_name", "city", "is_default"]
    list_filter = ["is_default", "province"]
    search_fields = ["full_name", "phone", "city", "province", "postal_code", "user__phone"]
...
@admin.register(User)
class UserAdmin(NormalizedSearchAdminMixin, BaseUserAdmin):
...
@admin.register(Otp)
class OtpAdmin(NormalizedSearchAdminMixin, admin.ModelAdmin):
    list_display = [
        "phone",
        "used",
        "attempts",
